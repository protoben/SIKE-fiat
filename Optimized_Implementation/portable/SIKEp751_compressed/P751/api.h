...
//          compressed public key pk          (CRYPTO_PUBLICKEYBYTES = 334 bytes) 
int crypto_kem_keypair(unsigned char *pk, unsigned char *sk);

// SIKE's encapsulation
// Input:   compressed public key pk          (CRYPTO_PUBLICKEYBYTES = 334 bytes)
// Outputs: shared secret ss                  (CRYPTO_BYTES = 32 bytes)
//          compressed ciphertext message ct  (CRYPTO_CIPHERTEXTBYTES = 363 bytes) 
...
// Public keys pk consist of 3 elements in Z_Bob_order plus one in GF(p610^2) plus 2 bytes. In the SIKE API, pk is encoded in 334 octets.
// Ciphertexts ct consist of the concatenation of a public key value and a 32-byte value. In the SIKE API, ct is encoded in 334 + 32 = 363 octets.  
// Shared keys ss consist of a value of 32 octets.


/*********************** Ephemeral key exchange API ***********************/

#define SIDH_SECRETKEYBYTES      47
...
// Compressed Public keys PublicKeyA and PublicKeyB consist of 3 elements in {Z_2^OALICE_BITS, Z_3^OBOB_EXPON}. In the SIDH API, they are encoded in 334 octets.
