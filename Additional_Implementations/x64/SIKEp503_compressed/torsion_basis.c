...
void get_2_torsion_entangled_basis_compression(const f2elm_t A, point_t S1, point_t S2, 
                                               unsigned char *bit, unsigned char *entry) 
{// Build an entangled basis for E[2^m].
 // At first glance this is similar to the Elligator 2 technique, but the field element u is a *square* here, not a non-square.
    ...
    *bit = isSqrA;  // This bit is transmitted along with the PK to speedup decompression
    ...
    *entry = (index - 2)/2;  // This table entry will also be transmitted along with the PubKey to speedup decompression
...
void CompleteMPoint(f2elm_t A, point_proj_t P, point_full_proj_t R)
